...
import { toJson } from './utils/objectUtils';
...
class dispatcherWSClient {
  ...
  activateLog = () => {
    this._client.on('flux server error', (err) => {
      console.log(err);
    });
  }
  
  /*
    Create a store on the flux socket.io server
  */
  createStore = (store) => {
    const storeString = toJson(store);
    this._client.emit('create store', this._clientID, storeString);
  }
