  ...
  subscribe = (...stores) => {
    stores.map(store => {
      ...
      if (!this._stores[store.name]) {
        ...
        console.log('Existing store : ' + store.name);
      }
    });
  ...
  _start = (io) => {
    io.on('connection', (socket) => {
      ...
      */
      ...
      /*
        Subscribe a store by the client
      */
      socket.on('create store', (clientID, storeString) => {
        const addStore = jsonToObject(storeString);
        this.subscribe(addStore);
        this._setStoreAction(socket, addStore);
      });
      ...
      */
      ...
      this._setStoreAction(socket, this._stores);
